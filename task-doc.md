...
CMinor 语言的源程序部分是 C 语言的子集，验证标注部分是 ACSL 语言的子集。

> 注意：秩函数是一个整型表达式，对于带有秩函数的源码文件，我们保证其文件中所有函数及循环都被标注了秩函数，且所有秩函数的元组长度均相同。
...
为了证明程序的**完全正确性**，还需要证明程序的终止性，即程序的执行路径都是有限的。为此，CMinor 语言还支持用于证明终止性的**秩函数**的标注。秩函数是一个表达式，在函数头以 `decreases` 标出，在循环头以 `loop variant` 标出。所有有效的秩函数标注的值都是**非负的**。同时，对于程序中的任意一条基本路径，在路径头标注的秩函数的值，都**严格大于**路径尾标注的秩函数的值。以此便可以证明程序的任意执行路径都是有限的。
